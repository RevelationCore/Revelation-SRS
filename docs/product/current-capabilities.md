...
| Attendance and academic engagement | **Partial** | Migration `0037`, Drizzle schema, RLS and value sets implement the storage baseline; expected-event creation/query, idempotent observation capture/correction and student timeline APIs publish typed events | Integration tests await a container runtime; no policy evaluation, intervention workflow or end-to-end application journey |
