...
| Attendance and academic engagement | **Partial** | Migrations `0037`–`0039` and APIs implement evidence capture/correction, approved policy evaluation, explainable alerts, intervention triage, contacts, actions, reviews, closure and minimum-necessary referrals | Integration tests await a container runtime; no admin UI, demo journey or approved UKVI read-model integration |
