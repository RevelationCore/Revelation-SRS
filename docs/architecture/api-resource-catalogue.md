...
| Timetable and academic engagement | `timetabled_activity`, `student_timetable_entry`, `expected_engagement_event`, `engagement_observation`, `engagement_observation_revision`, `engagement_policy_version`, `engagement_alert` | Public core API / integration API / workflow API | Evidence routes plus `POST /api/v1/engagement/policies`, `POST /api/v1/engagement/evaluations`, `GET /api/v1/engagement/alerts` | Expected activity remains distinct from observed evidence; policy version and immutable evidence hash explain every alert; unsafe evidence suspends automation. |
