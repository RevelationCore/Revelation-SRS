...
| Timetable and academic engagement | `expected_engagement_event`, `engagement_observation`, `engagement_policy_version`, `engagement_alert`, `engagement_intervention_case`, `engagement_contact_attempt`, `engagement_action`, `engagement_referral` | Public core API / integration API / workflow API | Evidence/policy routes plus alert triage and `/api/v1/engagement/cases/:caseId` contact, action and review operations | Unsafe evidence suspends automation; restricted referrals carry only allowlisted operational fields and never make academic-status or sponsor decisions. |
