#!/usr/bin/env bash
# scripts/create-uat-issues-remaining.sh
#
# Creates the remaining 42 UAT story issues (P-05, P-07, P-10, P-11, P-13,
# and all admin + cross-cutting stories).
# Milestone and labels already exist from the first run.
#
# Uses --body-file - (stdin heredoc) to avoid heredoc-in-$() parse issues.
...
echo "==> Portal stories (missing)"

new_issue "P-05: Browse module registrations" "uat/story,area/portal" <<'BODY'
...
If any step fails, open a new issue using the **UAT Bug Report** template and reference story **P-05** with the step number. Paste the link here as a comment.
BODY

new_issue "P-07: Add an optional module" "uat/story,area/portal" <<'BODY'
...
new_issue "P-10: Declare a disability" "uat/story,area/portal" <<'BODY'
...
new_issue "P-11: Submit an extenuating circumstances claim" "uat/story,area/portal" <<'BODY'
...
new_issue "P-13: View notifications" "uat/story,area/portal" <<'BODY'
> Alice notices the notifications item in her sidebar. She wants to check if she has any messages from the university and mark them as read.
...
echo "==> Admin — student records stories"

new_issue "AR-01: Search for a student and open their record" "uat/story,area/admin,area/student-records" <<'BODY'
...
new_issue "AR-02: Review student overview, enrolments, and contacts" "uat/story,area/admin,area/student-records" <<'BODY'
...
new_issue "AR-03: Review module registrations from the student record" "uat/story,area/admin,area/student-records" <<'BODY'
...
new_issue "AR-04: Review a student's assessment marks" "uat/story,area/admin,area/student-records" <<'BODY'
...
- [ ] **5.** A module result summary should show the aggregate mark and result code (Pass, Compensated, or Fail)
...
new_issue "AR-05: Review communications sent to a student" "uat/story,area/admin,area/student-records" <<'BODY'
...
- [ ] **5.** If no communications exist for the selected student, try a different student — or note it as an empty state and verify the empty state message is helpful
...
echo "==> Admin — corrections stories"

new_issue "CO-01: Raise a new correction case against a board decision" "uat/story,area/admin,area/corrections" <<'BODY'
...
new_issue "CO-02: Progress and resolve a correction case" "uat/story,area/admin,area/corrections" <<'BODY'
...
echo "==> Admin — wellbeing stories"

new_issue "WB-01: View and manage a student wellbeing referral" "uat/story,area/admin,area/wellbeing" <<'BODY'
...
new_issue "WB-02: Review a disability support case" "uat/story,area/admin,area/wellbeing" <<'BODY'
...
new_issue "WB-03: Process an extenuating circumstances submission" "uat/story,area/admin,area/wellbeing" <<'BODY'
...
echo "==> Admin — exam board stories"

new_issue "EB-01: Browse the exam boards list" "uat/story,area/admin,area/exam-boards" <<'BODY'
...
- [ ] **3.** A list of exam boards should appear — S5 has 4 boards (3 module boards and 1 award board)
...
new_issue "EB-02: Review an exam board — agenda and data pack" "uat/story,area/admin,area/exam-boards" <<'BODY'
...
- [ ] **6.** Board membership and attendees should be listed (members, external examiner sign-offs)
...
new_issue "EB-03: Review candidate profiles and progression decisions" "uat/story,area/admin,area/exam-boards" <<'BODY'
...
- [ ] **3.** A list of students considered at this board should appear — S5 has approximately 700 candidate profiles
...
new_issue "EB-04: Review ratified award decisions" "uat/story,area/admin,area/exam-boards" <<'BODY'
...
- [ ] **5.** The total count of awards should be visible (S5 has approximately 200 awards)
...
echo "==> Admin — task inbox story"

new_issue "TI-01: Work through the task inbox" "uat/story,area/admin" <<'BODY'
...
echo "==> Admin — regulatory stories"

new_issue "RE-01: Review the regulatory compliance overview" "uat/story,area/admin,area/regulatory" <<'BODY'
...
- [ ] **6.** All five sub-pages should load without an error
...
new_issue "RE-02: Review HESA return status and submissions" "uat/story,area/admin,area/regulatory" <<'BODY'
...
new_issue "RE-03: Review OfS regulatory obligations" "uat/story,area/admin,area/regulatory" <<'BODY'
...
new_issue "RE-04: Review SLC loan data and triggers" "uat/story,area/admin,area/regulatory" <<'BODY'
...
new_issue "RE-05: Review UCAS application pipeline data" "uat/story,area/admin,area/regulatory" <<'BODY'
...
- [ ] **4.** The data should be plausible for a large institution
...
new_issue "RE-06: Review UKVI compliance and CAS records" "uat/story,area/admin,area/regulatory" <<'BODY'
> The DPO needs to review the institution's UKVI compliance position — specifically checking CAS records and any flagged attendance issues.
...
- [ ] **5.** The data should be plausible (S6 has approximately 30 CAS records per cohort year)
...
new_issue "RE-07: Process an FOI / Subject Access Request" "uat/story,area/admin,area/regulatory" <<'BODY'
...
- [ ] **1.** Log in as `dpo` on S4 and navigate to **Reporting -> FOI**
- [ ] **2.** The FOI/SAR page should load without error
- [ ] **3.** A register of FOI and SAR requests should be visible (or a helpful empty state if none exist in S4)
...
echo "==> Admin — reporting stories"

new_issue "RP-01: Generate and view the enrolment report" "uat/story,area/admin,area/reporting" <<'BODY'
...
new_issue "RP-02: Browse the reporting hub" "uat/story,area/admin,area/reporting,uat/placeholder" <<'BODY'
...
- [ ] **3.** Click **Enrolment volumes** — the report page should load
...
echo "==> Admin — operations stories"

new_issue "OP-01: Manage value sets — view, edit, retire, and add members" "uat/story,area/admin,area/operations" <<'BODY'
...
- [ ] **2.** Navigate to **Tenant Admin -> Value Sets**
...
new_issue "OP-02: Manage feature flags" "uat/story,area/admin,area/operations" <<'BODY'
...
new_issue "OP-03: Browse the integration registry" "uat/story,area/admin,area/operations" <<'BODY'
...
- [ ] **4.** Click on an entry to see contract details if the interface allows it
...
new_issue "OP-04: Review integration operations and connector status" "uat/story,area/admin,area/operations" <<'BODY'
...
new_issue "OP-05: Review academic rules configuration" "uat/story,area/admin,area/operations" <<'BODY'
...
new_issue "OP-06: Review workflow definitions" "uat/story,area/admin,area/operations" <<'BODY'
...
new_issue "OP-07: Globalisation and locale settings" "uat/story,area/admin,area/operations" <<'BODY'
...
new_issue "OP-08: View environment runtime information" "uat/story,area/admin,area/operations,uat/placeholder" <<'BODY'
...
> **Note:** The admin dashboard quick-search is **[PLACEHOLDER — not yet wired]**. Stat cards show `—`. This is expected. The Environment Runtime page is fully functional.
...
- [ ] **2.** The page should display the current release version
...
- [ ] **7.** Also visit the **Dashboard** and note the stat cards show `—` and the search is not wired — confirm this matches the known placeholder status
...
new_issue "OP-09: Tenant administration and configuration" "uat/story,area/admin,area/operations" <<'BODY'
...
- [ ] **5.** Update a non-critical field and save — confirm the change persists on refresh
- [ ] **6.** Also navigate to the **Tenant Admin hub** and confirm all eight sub-section cards are visible and link to the correct pages
...
echo "==> Admin — audit story"

new_issue "AU-01: Browse the system audit log" "uat/story,area/admin,area/audit" <<'BODY'
...
echo "==> Cross-cutting stories"

new_issue "X-01: Demo environment reset and scenario banner verification" "uat/story,area/admin,area/portal" <<'BODY'
...
- [ ] **1.** Run the reset command to load a known scenario: `DATABASE_URL=postgres://srs:srs@localhost:5432/srs pnpm --filter @revelation-srs/demo-data demo:reset enrolment-induction`
- [ ] **2.** Wait for the **"Reset complete"** or **"Migrations applied successfully"** output before proceeding
- [ ] **3.** Open the admin app at http://localhost:5173 and log in — the demo banner should read **enrolment-induction**
- [ ] **4.** Navigate to **Students** — student data should reflect the S2 dataset
- [ ] **5.** Now reset to S5: `DATABASE_URL=postgres://srs:srs@localhost:5432/srs pnpm --filter @revelation-srs/demo-data demo:reset exam-board`
- [ ] **6.** Refresh both apps — banners in both should update to **exam-board**
- [ ] **7.** Log in to the portal as `alice.demo` — the banner should show **exam-board**
- [ ] **8.** There should be no residual data from the previous scenario visible in any views
...
new_issue "X-02: Admin sidebar navigation — all sections reachable" "uat/story,area/admin,uat/placeholder" <<'BODY'
...
Work through each sidebar section in order. Each page should load without a blank screen, a spinner that never resolves, or an unhandled error.
...
new_issue "X-03: Accessibility statements and error pages" "uat/story,area/admin,area/portal" <<'BODY'
...
**Portal**
- [ ] **1.** Open http://localhost:5174/accessibility-statement
...
- [ ] **5.** Log out of the portal, then navigate directly to http://localhost:5174/dashboard — you should be redirected to the login page
...
- [ ] **10.** Log out of the admin app, then navigate directly to http://localhost:5173/students — you should be redirected to the login page
...
echo ""
echo "==> All 42 remaining issues created."
echo "    View milestone: https://github.com/RevelationCore/Revelation-SRS/milestone/1"
