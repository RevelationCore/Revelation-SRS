 ...
 */
export function monetaryAmountToNumber(amount: MonetaryAmount): number {
  if (amount.minorUnits === 0) return Number(amount.amount);
 ...
 */
export function monetaryAmountFromNumber(
  value: number,
  currencyCode: string,
  minorUnits: number,
): MonetaryAmount {
