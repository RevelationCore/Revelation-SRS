/**
 * AttendanceEventConsumer — NATS JetStream consumer for SRS context events.
 *
 * Connects to NATS, subscribes to the SRS_EVENTS stream with a durable push
 * consumer, and dispatches messages to the appropriate projection handler.
 * Each message is processed inside a single database transaction that also
 * records the event in the event_log table for idempotency.
 *
...
import type { AttendanceDb, AttendanceTx } from '../db/client.js';
...
const CONSUMER_NAME = 'attendance-context-consumer';
...
export class AttendanceEventConsumer {
  ...
  async connect(): Promise<void> {
    ...
    this.log.info({ natsUrl: this.natsUrl }, 'Attendance NATS consumer connected');
  ...
  async start(): Promise<void> {
    ...
    this.log.info({ consumer: CONSUMER_NAME }, 'Attendance event consumer started');
  ...
  async dispatch(
    ...
    await withAttendanceTenantContext(this.db, envelope.tenantId, async (tx) => {
  ...
  private async ensureConsumer(nc: NatsConnection): Promise<void> {
    ...
    // here we just verify — the attendance module does not own the stream).
    try {
...
export async function routeToHandler(
  ...
  switch (envelope.type) {
    ...
    case EVENT_TYPES.ENROLMENT_MODULE_REGISTRATION_COMPLETED:
      return handleModuleRegistrationCompleted(tx, envelope as DomainEventEnvelope<any>);
    default:
