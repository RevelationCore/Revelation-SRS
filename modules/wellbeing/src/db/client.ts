...
export type WellbeingDb = ReturnType<typeof createWellbeingDb>;
export type WellbeingTx = Parameters<Parameters<WellbeingDb['transaction']>[0]>[0];

export function createWellbeingDb(connectionString: string): ReturnType<typeof drizzle<typeof schema>> {
 ...
 *
 * Typed for WellbeingDb rather than Db, so it cannot be confused with the
 * SRS database's withTenantContext helper in packages/db.
 */
export async function withWellbeingTenantContext<T>(
  db: WellbeingDb,
  tenantId: string,
  fn: (tx: WellbeingTx) => Promise<T>,
): Promise<T> {
