...
describe('admin demo context', () => {
  it('keeps Alpha, scenario, version and limitations visible', async () => {
    server.use(
      http.get('http://localhost:3000/api/v1/demo/status', () =>
        HttpResponse.json({ active: true, scenarioName: 'Exam Board', scenarioSlug: 'exam-board', schemaVersion: '0048', demoNow: '2026-06-01T00:00:00Z', nextResetAt: null }),
      ),
    );
    const { container } = render(<DemoBanner />);
    expect(await screen.findByText('Alpha demo')).toBeVisible();
    expect(screen.getByText('Scenario: exam-board')).toBeVisible();
    expect(screen.getByText('Data v0048')).toBeVisible();
